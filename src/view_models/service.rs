...
use crate::models::service::ServiceModel;
use crate::repos::service::ServiceRepo;
use crate::models::settings::{SettingsModel, FetchMode};

#[derive(Clone, Debug)]
pub enum Message {
    Load(Option<String>),
    Loaded(Result<Vec<ServiceModel>, Arc<kube::Error>>),
    Select(usize),
    Delete,
    Deleted(Result<(), Arc<kube::Error>>),
    Tick,
    WatchEvent(crate::kubernetes::KubeWatchEvent<ServiceModel>),
}

pub struct ServiceViewModel {
    pub items: Vec<ServiceModel>,
    pub selected_index: Option<usize>,
    pub loading: bool,
    pub error: Option<String>,
}

impl ServiceViewModel {
    pub fn new() -> Self {
    ...
    pub fn subscription(&self, settings: &SettingsModel, _namespace: Option<String>, client: std::sync::Arc<Client>) -> Subscription<Message> {
        let mode = settings.get_fetch_mode("Services");
        match mode {
            FetchMode::Polling => {
            ...
            FetchMode::Watcher => {
                crate::kubernetes::watch_namespaced_resource::<k8s_openapi::api::core::v1::Service, _, _>(
                    ("services_watch", _namespace.clone()),
                    client,
                    _namespace,
                    |evt| {
                        let m = match evt {
        ...
        match message {
            Message::Load(namespace) => {
                ...
                if let Some(c) = client {
                    ...
                    Task::perform(
                        async move { ServiceRepo::list(&c, ns.as_deref()).await.map(|items| items.into_iter().map(Into::into).collect()) },
            ...
            Message::Delete => {
                if let (Some(idx), Some(c)) = (self.selected_index, client) {
                    if let Some(item) = self.items.get(idx).cloned() {
                        return Task::perform(
                            async move { ServiceRepo::delete(&c, &item.name, &item.namespace).await },
