...
pub enum Message {
    Load(Option<String>),
    Loaded(Result<Vec<DeploymentModel>, Arc<kube::Error>>),
    Select(usize),
    Delete,
        ...
        match message {
            Message::Load(namespace) => {
                ...
                if let Some(c) = client {
                    ...
                    Task::perform(
                        async move { DeploymentRepo::list(&c, ns.as_deref()).await.map(|items| items.into_iter().map(Into::into).collect()) },
            ...
            Message::Delete => {
                if let (Some(idx), Some(c)) = (self.selected_index, client) {
                    if let Some(item) = self.items.get(idx).cloned() {
                        return Task::perform(
                            async move { DeploymentRepo::delete(&c, &item.name, &item.namespace).await },
