...
use crate::view_models::ingress::{IngressViewModel, Message};

pub fn view(vm: &IngressViewModel) -> Element<Message> {
    if vm.loading && vm.items.is_empty() {
        return container(text("Loading Ingresses...").size(20)).center_x(Length::Fill).center_y(Length::Fill).into();
    }
