...
use crate::view_models::node::{NodeViewModel, Message};

pub fn view(vm: &NodeViewModel) -> Element<Message> {
    if vm.loading && vm.items.is_empty() {
        return container(text("Loading...").size(20)).center_x(Length::Fill).center_y(Length::Fill).into();
    }
    ...
    for (i, item) in vm.items.iter().enumerate() {
        let content = row![
            ...
            text(&item.age).width(Length::FillPortion(1)),
        ].spacing(10);
