...
use crate::view_models::deployment::{DeploymentViewModel, Message};

pub fn view(vm: &DeploymentViewModel) -> Element<Message> {
    if vm.loading && vm.items.is_empty() {
        return container(text("Loading Deployments...").size(20)).center_x(Length::Fill).center_y(Length::Fill).into();
    }
