...
use crate::view_models::event::{EventViewModel, Message};

pub fn view(vm: &EventViewModel) -> Element<Message> {
    if vm.loading && vm.items.is_empty() {
        return container(text("Loading Events...").size(20)).center_x(Length::Fill).center_y(Length::Fill).into();
    }
